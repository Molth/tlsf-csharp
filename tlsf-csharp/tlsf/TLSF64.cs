...
namespace tlsf
{
    public static unsafe class TLSF64
    {
        ...
        {
            _ = user;
            Console.WriteLine("\t{0} {1} size: {2:X} ({3})", (nuint)ptr, used != 0 ? "used" : "free", (uint)size, (nuint)block_from_ptr(ptr));
        }
        ...
        public static void* tlsf_add_pool(void* tlsf, void* mem, ulong bytes)
        {
            ...
            {
                Console.WriteLine("tlsf_add_pool: Memory size must be between 0x{0:X} and 0x{1:X}00 bytes.", (uint)(pool_overhead + block_size_min), (uint)((pool_overhead + block_size_max) / 256));
                return null;
            }
